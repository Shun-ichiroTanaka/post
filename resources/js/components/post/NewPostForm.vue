    ...
    <div id="app">
        <form action="post.newpost" method="POST" @submit.prevent="newpost()">
            <div class="c-post__new-title">
                <input id="title" placeholder=" タイトル（例：「1日5分!マクロ経済のオススメ勉強法」）" 　type="text" v-model="title" required>
            </div>
            <div class="c-post__new-box">
                <div class="c-post__new-box__tags">
                    ...
                    <tags-input
                        ...
                        v-model="tags"
                        :existing-tags="[{ key: 'web-development', value: 'Web Development' },{ key: 'php', value: 'PHP' },{ key: 'javascript', value: 'JavaScript' },{ key: 'ruby', value: 'Ruby' },{ key: 'python', value: 'Python' },{ key: 'マクロ経済学', value: 'マクロ経済学' },{ key: 'ミクロ経済学', value: 'ミクロ経済学' },{ key: 'ゲーム理論', value: 'ゲーム理論' },{ key: 'html', value: 'HTML' },{ key: 'css', value: 'CSS' },]"
            ...
            <form-wizard @on-complete="onComplete" color="#4BCFC8" transition="bounce" back-button-text="戻る" next-button-text="追加" finish-button-text="これ以上追加できません">
                <tab-content title="Step1">
                    ...
                    <vue-editor v-model="step1" class="form-control" id="editor1" useCustomImageHandler @image-added="handleImageAdded" placeholder="Stepは最大4つまで登録できます（例：「STEP1：まずはマクロ経済をザックリ理解しよう」）"></vue-editor>
                </tab-content>
                <tab-content title="Step2">
                    <vue-editor v-model="step2" class="form-control" id="editor2" useCustomImageHandler @image-added="handleImageAdded" placeholder="（例：「STEP2：次に必須の公式を押さえよう」）"> </vue-editor>
                </tab-content>
                <tab-content title="Step3">
                    <vue-editor v-model="step3" class="form-control" id="editor3" useCustomImageHandler @image-added="handleImageAdded" placeholder="（例：「STEP3：頻出の問題系統を押さえよう」）"> </vue-editor>
                </tab-content>
                <tab-content title="Last Step" icon="fas fa-check">
                    <vue-editor v-model="step4" class="form-control" id="editor4" useCustomImageHandler @image-added="handleImageAdded" placeholder="（例：「STEP4：試験までにこなしておくべき参考書一覧」）"></vue-editor>
                </tab-content>
            </form-wizard>
